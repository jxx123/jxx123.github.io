...
A word on who the Mohists were, because it bears on how seriously to take their taxonomy. Mozi (墨子, c. 470–391 BCE) was a carpenter by trade, sought out by rulers as an expert on fortification,[^mozi-carpenter] and he has a fair claim to being the earliest roboticist on record: the *Han Feizi* has him spend three years on a wooden bird that flew for a day, a generation before Archytas's dove.[^mozi-bird] The one rival claim is his contemporary Lu Ban — and the *Mozi* records Mozi beating Lu Ban's cloud-ladder siege engine nine times running in a tabletop war game, after which the attack on Song was called off.[^mozi-gongshu] His followers became the siege-defence engineers of the Warring States, and the Canon they left behind is the earliest Chinese text on optics, including the first description of the camera obscura.[^mozi-canon]

Nor was this a marginal sect. Among the 百家争鸣, the Hundred Schools contending across the Spring and Autumn and Warring States centuries (roughly 771–221 BCE), Mohism was one of the four main schools and Confucianism's chief rival; Mencius complained that the words of Yang Zhu and Mozi "fill the world."[^mozi-mencius] It collapsed only after the Qin unification, when the siege warfare it had mastered ceased to exist.[^mozi-decline] When the Mohists sort knowledge by its source, that is the sorting of people who built things that had to work.
</aside>
...
### 说 is scaled, but not in the loop

It is worth being concrete about how 说 is and is not scaled, because the field looks busier on this axis than it is. A vision-language-action model — a VLA, the standard robot policy architecture — maps observation to action. There is no deliberation anywhere in that loop: the model that acts does not reason about what its action is about to do. The standard fix has been to bolt a System 2 on top — a separate reasoning model that plans, hands a subgoal down to a System 1 policy, and then gets out of the way. **Two standalone models, with the thinking sitting upstream of the acting rather than inside it.**

And that System 2 *is* being scaled. [Gemini Robotics-ER](https://deepmind.google/models/gemini-robotics/){:target="_blank"} is a reasoning model built for exactly this, there are success stories of GPT-6 and Claude controlling robots, and a steady stream of posts in which a VLM out-scores the action model underneath it. Take that seriously: it is a sign of life, the first evidence that spending more deliberation on a robot buys more mileage — even if the way it wins is a long run of tool calls, several rewrites of the code, and trial and error until something sticks.

But look at what is *not* being scaled in that picture. The thinking is scaled with the action out of the loop: System 2 never sees the result of an action as it happens; it sends a plan down and, seconds later, gets a report back. And it is scaled over a *frozen* System 1: the reasoning model gets bigger every quarter, and the policy underneath it learns nothing from any of it — the two are never improved at the same time. That is worse than it sounds. When the policy cannot do the thing — pick forks off a floor, say — a stronger System 2 does not make it able. It reroutes: a way around the incompetence rather than through it, and the more capable the thinker, the better it gets at routing around the hands. Scaling System 2 over a frozen System 1 does not close the gap; it learns to avoid the gap, and locks it in. Underneath is the interface: thinker and actor share no representation — one emits words or code, the other consumes pixels and emits torques — and how the two get glued fluidly, or how action and video prediction become part of the thinking itself, is barely investigated.

Which is why nothing caught the basket. Whatever System 2 was involved had finished before the load shifted, and System 1 does not think. The precise claim is not that nobody scales thinking; it is that nobody scales 说 *with the action in the loop*. We are scaling a preamble.

And even where the slow loop wins, the win does not compound. When a frontier model finally gets the forks off the floor, the trace it leaves is a pile of discarded code, not a trajectory in the policy's own action space, so nothing is distilled back into the fast model and the next time is just as slow. The pattern that works elsewhere is AlphaZero's: slow search produces targets, the fast network amortizes them, the next search starts from the better prior. Robotics has the slow side now and no return path. Once slow thinking has solved an unseen task, that experience has to become instinct — no slow thinking the second time. Until the slow solver writes in the policy's language, every unseen task stays expensive forever.
...
[^mozi-bird]: *Han Feizi*, 外儲說左上: 「墨子為木鳶，三年而成，蜚一日而敗。」 — Mozi made a wooden kite; three years to build, it flew for one day and broke. The *Mozi* itself (魯問) gives the bird to Lu Ban instead — 「公輸子削竹木以為鵲，成而飛之，三日不下」, aloft three days — and has Mozi dismiss it as less useful than a cart linchpin. Archytas of Tarentum (c. 428–347 BCE), the usual Western candidate for the first automaton, is credited with a flying wooden dove by Aulus Gellius, writing five centuries later. All of these accounts are late, so "earliest" is a claim about tradition, not archaeology.
[^mozi-gongshu]: *Mozi*, chapter 公輸 ("Gongshu"). Mozi "engaged in nine simulated war games with Gongshu Ban" and "overturned each one of his stratagems." See [Mozi](https://en.wikipedia.org/wiki/Mozi){:target="_blank"}, Wikipedia. Gongshu Ban is [Lu Ban](https://en.wikipedia.org/wiki/Lu_Ban){:target="_blank"} (c. 507–444 BCE), revered since as the patron deity of builders and carpenters — the other candidate for the greatest engineer of the age, which is what makes the match worth recording.
