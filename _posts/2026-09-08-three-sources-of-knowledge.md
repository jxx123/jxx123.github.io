...
The fair objection is that System 2 *is* being scaled. [Gemini Robotics-ER](https://deepmind.google/models/gemini-robotics/){:target="_blank"} is a reasoning model built for exactly this, there are success stories of GPT-6 and Claude controlling robots, and a steady stream of posts in which a VLM out-scores the action model underneath it. Take that seriously: it is a sign of life, the first evidence that spending more deliberation on a robot buys more mileage. But watch *how* they win — a long run of tool calls, several rewrites of the code, trial and error until something sticks — and watch what they cannot do. A frontier model driving the basket would probably notice the forks and rethink; then the hands under it cannot carry out the new plan, because code-as-policy has to start its trial and error over and the VLA was never trained to pick forks off a floor. Thinker and actor share no representation — one emits words or code, the other consumes pixels and emits torques — and how the two get glued fluidly, or how action and video prediction become part of the thinking itself, is barely investigated.

Which is why nothing caught the basket. The precise claim is not that nobody scales thinking; it is *when* the thinking runs relative to the acting. Whatever System 2 was involved had finished before the load shifted, and System 1 does not think. Nobody is scaling 说 *in the loop*; we are scaling a preamble.
